/****** Object:  TableFunction [dbo].[LayoutSQLParts_PivotWork]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE function dbo.LayoutSQLParts_PivotWork
(
					...
					--+ ', ' + case when ltrim(SrcName) = '' then '''''' else SrcName end + ')'
						...
						+ case when coalesce(LayoutFormatID,0) = 0 then
							...
							case when coalesce(outwidth,0) > 0 then
								...
								+ ', WiredJobControlRun.FuncLib.ApplyLayoutFormat(' + case when ltrim(SrcName) = '' then '''''' else '[' + SrcName + ']' end + ',' + ltrim(str(LayoutFormatID)) + ')),'''')'
							else
								'coalesce(WiredJobControlRun.FuncLib.ApplyLayoutFormat(' + case when ltrim(SrcName) = '' then '''''' else '[' + SrcName + ']' end + ',' + ltrim(str(LayoutFormatID)) + '),'''')'
							end
						end
			...
			begin
				...
				BEGIN
					if @QueryTemplateID = -2 --Pivot
						set @FldSrc = 'convert(varchar(1000),''0'') [' + @OutName + ']'
					else
