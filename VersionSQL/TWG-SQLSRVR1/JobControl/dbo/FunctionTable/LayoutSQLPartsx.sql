/****** Object:  TableFunction [dbo].[LayoutSQLPartsx]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE function dbo.LayoutSQLPartsx
(
	...
	if @JobStepID is not null
		...
		begin
		 ...
		 DECLARE TransTempCol CURSOR FOR
			...
			--SELECT case when coalesce(CaseChange,'None') = 'upper' then 'upper(' 
			...
			--select case when ltrim(SrcName) = '' then '''''' else '[' + SrcName + ']' end + ' [' + OutName+ ']' FldSrc
			select case when coalesce(DoNotTrim, 0) = 0 then
					'ltrim(rtrim(coalesce(convert(' + case when coalesce(outwidth,0) > 0 then 'varchar(' + ltrim(str(coalesce(outwidth,50))) + ')' else 'varchar(1000)' end
							+ ', ' + case when ltrim(SrcName) = '' then '''''' else '[' + SrcName + ']' end + '),''''))) [' + OutName+ ']'
					else 
					'coalesce(convert(' + case when coalesce(outwidth,0) > 0 then 'varchar(' + ltrim(str(coalesce(outwidth,50))) + ')' else 'varchar(1000)' end
							+ ', ' + case when ltrim(SrcName) = '' then '''''' else '[' + SrcName + ']' end + '),'''') [' + OutName+ ']'
					end FldSrc
...
                    --!! and the corresponding controls in XForm/Layout control
		 ...
		 if @QueryTemplateID <= 0
			 ...
			 begin
						-- 1076, 1077 = Stats
			    if @QueryTemplateID in (1076,1077,1080,1095,1099) or @QueryTypeID = 24 or @QuerySourceID = 2   -- Queries without  ModificationType
					begin
						if @OrderClause > ''
