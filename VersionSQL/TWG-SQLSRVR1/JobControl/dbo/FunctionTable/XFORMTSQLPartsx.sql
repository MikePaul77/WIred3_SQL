/****** Object:  TableFunction [dbo].[XFORMTSQLPartsx]    Committed by VersionSQL https://www.versionsql.com ******/

create FUNCTION dbo.XFORMTSQLPartsx 
(
...
BEGIN
   ...
   declare @sep VARCHAR(10) = ''
	...
	begin
		...
		BEGIN
			...
			if @FldSrc like '%#IssueWeek#%'
				set @FldSrc = replace(@FldSrc, '#IssueWeek#', ltrim(str(@IssueWeek)))
			...
			begin
				...
				if @CalcMax = 1
					set @CalcFldSrc = @CalcFldSrc + ', Max(Convert(' + @PivotCalcType + ',' + @FldSrcName + ')) XF_' + @FldSrcOut + '_Max'
				if @CalcMin = 1
					set @CalcFldSrc = @CalcFldSrc + ', Min(Convert(' + @PivotCalcType + ',' + @FldSrcName + ')) XF_' + @FldSrcOut + '_Min'
				if @CalcAvg = 1
					set @CalcFldSrc = @CalcFldSrc + ', Avg(Convert(' + @PivotCalcType + ',' + @FldSrcName + ')) XF_' + @FldSrcOut + '_Avg'
				if @CalcCount = 1
	...
	insert @XFormParts (FromClause
		...
		select case when coalesce(ltrim(@FromClause),'') = '' then ''
					when @QueryTemplateID < 0 then @FromClause
					when @JobStepID is not null then @WorkDB + '..' + @SrcTable
						else @FromClause
					end FromClause
				, case when coalesce(ltrim(@FromClause),'') = '' then '' else @JoinClause end JoinClause
				...
				, case when coalesce(ltrim(@FromClause),'') = '' then ''
						when @JobStepID is not null then
							'select #SourceFields# #FieldList1# #FieldList2# #FieldList3# #FieldList4# #IntoClause# From #FromClause# x #JoinClause# #WhereClause# '
						when @XFormFieldID is not null then 
							'select top 100 #FieldList1# From #FromClause# x #JoinClause# #WhereClause# '
						else 
							'select top 100 #SourceFields# #FieldList1# #FieldList2# #FieldList3# #FieldList4# #IntoClause# From #FromClause# x #JoinClause# #WhereClause# '
					end ClauseFrameWork
