/****** Object:  Procedure [dbo].[GetAttachableFiles]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE dbo.GetAttachableFiles @UseID int, @Mode varchar(50)
AS
BEGIN
	SET NOCOUNT ON;
	...
	if @Mode = 'AddedFiles' or @Mode = 'AddedFilesPlus'
		...
		BEGIN
			...
			if @Mode = 'AddedFilesPlus'
				begin
					select fd.DocumentID, fd.FileName
						from #FullDocsAF fd
                        ...
                        from (
							select js.JobID, j.JobName, js.StepFileNameAlgorithm ZipOutName
								...
								from JobSteps js
									...
									join (select JobID, min(StartedStamp) JobStartedStamp
											from JobControl..JobSteps 
											where coalesce(Disabled,0) = 0 
											group by JobID) Fjs on js.JobID = Fjs.JobID
...
--									and ((dm.FileCreateDate>=Dateadd(hh,-48,getdate())
									and ((dm.FileCreateDate>=Fjs.JobStartedStamp
											...
											and ((isnull(j.TestMode,0)=1 and dm.OK2Release = 0)
													or (dm.OK2Release = 1)))

										or dm.FileCreateDate is null)

							) x
                            left outer join (select s.code StateCode, s.StateFIPS + c.CountyFIPS FullFIPS
                                                from Lookups..States s
                                                    join Lookups..COunties c on c.StateID = s.id ) sf on x.WorkFileName like '%\_' + sf.FullFIPS + '\_%' ESCAPE '\'
						where x.DocSeq = 1
						...
						select js.JobID, j.JobName, js.StepFileNameAlgorithm ZipOutName
							...
							from JobSteps js
								...
								join (select JobID, min(StartedStamp) JobStartedStamp
											...
											group by JobID) Fjs on js.JobID = Fjs.JobID
							where js.StepID = @UseID
								--and (dm.JobLogID = (select max(ID) from JobControl..JobLog where JobID = js.jobID))  -- MP 9/2/2020 added becuase of the documentID's changes above we getting multiple version of the files from previous job Runs
								and (dm.JobLogID = (select top 1 ID from JobControl..JobLog where JobID = js.jobID and starttime is not null order by starttime desc))  -- MP 9/2/2020 added becuase of the documentID's changes above we getting multiple version of the files from previous job Runs
											--or vdm.DocumentID > 0)		-- MikeP 4/3/23 Removed this beacuse it was causing extra files to be added if the added reports were run more then once
								and (j.LastJobParameters = jl.JobParameters
										or (dm.FileCreateDate is null and jl.JobParameters is null))
