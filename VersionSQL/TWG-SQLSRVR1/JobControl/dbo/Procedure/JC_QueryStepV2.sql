/****** Object:  Procedure [dbo].[JC_QueryStepV2]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE dbo.JC_QueryStepV2 @UseJobStepID int
AS
BEGIN
	SET NOCOUNT ON;
							...
							Begin
								...
								begin
									...
									begin
										...
										declare @UseVal varchar(100) = convert(varchar, @SCTval)
										...
										if @QuerySourceID = 2 and @SCT = 'Town'
										Begin
											set @UseField = 'FIPSwTownName'
											select @UseVal = '''' + s.StateFIPS + CountyFIPS + t.FullName + ''''
													from Lookups..Towns t
														join Lookups..Counties c on c.Id = t.CountyId
														join Lookups..States s on s.ID = c.StateID
													 where t.ID = @SCTval
										end
										if @QuerySourceID = 3 and @SCT = 'County'
										Begin
											set @UseField = 'FIPS Code'
											select @UseVal = '''' + s.StateFIPS + CountyFIPS + ''''
													from Lookups..Counties c
														join Lookups..States s on s.ID = c.StateID
													 where c.ID = @SCTval
