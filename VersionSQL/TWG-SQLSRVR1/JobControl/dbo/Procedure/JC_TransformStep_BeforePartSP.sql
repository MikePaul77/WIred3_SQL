/****** Object:  Procedure [dbo].[JC_TransformStep_BeforePartSP]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[JC_TransformStep] @JobStepID int
AS
BEGIN
	SET NOCOUNT ON;
	...
	begin try
		...
		if @JobStepID < 0
			begin
				-- This is used to test the building of a Transform without a Job
				-- A negitive JobStep Parameter will be read as Read as TransformationTemplate ID to Test
				-- Will create 5K or less from the soure that will be trasformed and Layed out for evaluation
				declare @QueryTemplateSrc varchar(500)

				select @QueryTemplateSrc = qt.FromPart
						, @SrcTable = 'TransformTest_QuerySrc_' + ltrim(str(tt.ID)) 
						, @StepTable = 'TransformTest_ResultsA_' + ltrim(str(tt.ID)) 
						, @DestTable = 'TransformTest_Results_' + ltrim(str(tt.ID)) 
						, @TemplateID = tt.ID
						, @FileLayoutID = fl.ID
					...
					where tt.Id = abs(@JobStepID)

				set @cmd = 'IF OBJECT_ID(''JobControlWork..' + @SrcTable + ''', ''U'') IS not NULL drop TABLE JobControlWork..' + @SrcTable + ' '
				exec(@cmd)

				set @cmd = 'select top 100 * into JobControlWork..' + @SrcTable + ' from ' + @QueryTemplateSrc
				print @cmd
				exec(@cmd)
		 ...
		 DECLARE TransTempCol CURSOR FOR
			...
			SELECT coalesce(SourceCode,'null') + ' [XF_' + OutName+ ']' FldSrc
		 ...
		 BEGIN
			if @FldSrc like '%#JobStepID#%'
				set @FldSrc = replace(@FldSrc, '#JobStepID#', ltrim(str(@JobStepID)))
