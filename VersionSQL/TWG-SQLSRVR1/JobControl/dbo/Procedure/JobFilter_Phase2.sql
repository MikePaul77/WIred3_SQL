/****** Object:  Procedure [dbo].[JobFilter_Phase2]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE dbo.JobFilter_Phase2 @Mode varchar(10), @ShowDisabled bit
AS
BEGIN
	SET NOCOUNT ON;

-- All MTDE Comp Jobs Not in MA but Core (NH, CT, RI, ME and VT)

	if ltrim(rtrim(coalesce(@Mode,''))) = ''
		set @Mode = 'IDList'
	if @ShowDisabled is null
		set @ShowDisabled = 0
			...
			select j.jobID, j.JobName, j.disabled, j.vfpJobID
				...
				from  JobControl..Jobs j 
					...
					join ( select distinct j.JobID, ql.stateID
								...
								where ql.stateID > 0
									and s.Code in ('NH','CT','RI','ME','VT')) js on js.JobID = j.JobID
		...
		full outer join (
			select j.JOBID, j.JOBDESCRIP, j.Status, CURSTATE, c.PCEMAIL, c.PCFNAME, c.PCLNAME
				...
				where (' '  + replace(JOBDESCRIP,'-',' ') like '% Comp %' or ' '  + replace(JOBDESCRIP,'-',' ') like '% zzComp %'   ) 
						...
						and CURSTATE in ('NH','CT','RI','ME','VT')
