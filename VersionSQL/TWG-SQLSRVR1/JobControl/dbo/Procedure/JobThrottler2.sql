/****** Object:  Procedure [dbo].[JobThrottler2]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE dbo.JobThrottler2 
AS
BEGIN
	SET NOCOUNT ON;

	declare @StartStamp datetime = getdate()
	...
	declare @DeleteCompletedJobs bit = 0

	select @DeleteCompletedJobs = case when Value = '1' then 1 else 0 end
		from JobControl..Settings
		where Property = 'DeleteCompletedJobs' and coalesce(Disabled,0) = 0

	if @DeleteCompletedJobs = 1
		print '@DeleteCompletedJobs is ON ' 
	else
		print '@DeleteCompletedJobs is OFF ' 
	...
	if @tEnabled=1
		Begin
			...
			Print '@WaitingJobs: ' + ltrim(str(@WaitingJobs))
			Print '@RunningJob: ' + ltrim(str(@RunningJobs))
			...
			begin
				...
				print 'JobControl..StartWiredJob2'
				--exec JobControl..StartWiredJob @NextJobID, @JobLogRecID
				exec JobControl..StartWiredJob2 @NextJobID, @JobLogRecID
			...
			if 1=2 --thinking we don't need this any more
			begin
				print 'CHECK TO SEE IF ANY JOBS ARE RUNNING PAST THE MAXIMUM TIME LIMIT'
