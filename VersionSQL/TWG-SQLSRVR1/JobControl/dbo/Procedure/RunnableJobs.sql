/****** Object:  Procedure [dbo].[RunnableJobs]    Committed by VersionSQL https://www.versionsql.com ******/
...
BEGIN
	...
	select distinct j.JobID, j.JobName, j.RecurringTypeID, j.RecurringTargetID, j.RecurringOffset, ql.StateID
		...
		from JobControl..Jobs j
			...
			left outer join JobControl..JobSteps js on js.JobID = j.JobID and js.StepTypeID in (21,1124)
	...
	select x.JobID, x.LastRun, x.AdjustIW
		from (
			select JobID, coalesce(ParamValue,'None') LastRun
					, Sum(case when aj.StateID > 0 then 1 else 0 end) TotStates
					, Sum(case when JobControl.dbo.OffsetParam(aj.ParamName, @CurrIW, aj.RecurringOffset) <= iw.IssueWeek then 1 else 0 end) StateDataReady 
					, @CurrIW CurrentIW
					, Max(JobControl.dbo.OffsetParam(aj.ParamName, @CurrIW, aj.RecurringOffset)) AdjustIW
	...
	select x.JobID, x.LastRun, x.AdjustIM
		from (
			select JobID, coalesce(ParamValue,'None') LastRun
					, Sum(case when aj.StateID > 0 then 1 else 0 end) TotStates
					, Sum(case when JobControl.dbo.OffsetParam(aj.ParamName, @CurrIM, aj.RecurringOffset) <= im.IssueMonth then 1 else 0 end) StateDataReady 
					, @CurrIM CurrentIM
					, Max(JobControl.dbo.OffsetParam(aj.ParamName, @CurrIM, aj.RecurringOffset)) AdjustIM
					, Max(aj.RecurringTargetID) RecurringTargetID
					, Max(case when aj.AllStdStates = 1 then 1 else 0 end) AllStdStates
					, Max(coalesce(LastLiveStartTime,'1/1/2000')) LastLiveStartTime
				from #ActJobs aj
	...
	select x.JobID, x.LastRun, x.AdjustIM
		from (
			select JobID, coalesce(ParamValue,'None') LastRun
					, Sum(case when aj.StateID > 0 then 1 else 0 end) TotStates
					, Sum(case when JobControl.dbo.OffsetParam(aj.ParamName, @CurrCY, aj.RecurringOffset) <= CY.IssueMonth then 1 else 0 end) StateDataReady 
					, @CurrCY CurrentCY
					, Max(JobControl.dbo.OffsetParam(aj.ParamName, @CurrCY, aj.RecurringOffset)) AdjustIM
					, Max(aj.RecurringTargetID) RecurringTargetID
					, Max(aj.RecurringTypeID) RecurringTypeID
					, Max(case when aj.AllStdStates = 1 then 1 else 0 end) AllStdStates
					, Max(coalesce(LastLiveStartTime,'1/1/2000')) LastLiveStartTime
				from #ActJobs aj
					left outer join #CY CY on aj.StateID = CY.StateID 
				where RecurringTypeID in (4,5,6,8,9,10)
					and (aj.NextParamTarget <= @CurrCY
							or (aj.JobID = 2849 and aj.NextParamTarget <= @CurrCYPlus1))
				group by JobID, ParamValue
		) x
		where case when (TotStates = StateDataReady or AllStdStates = 1) and LastRun <> AdjustIM then 1 
					when (TotStates = StateDataReady or AllStdStates = 1) and JobID = 2849 and LastRun <> @CurrCYPlus1 then 1 
				else 0 end = 1
			and (RecurringTypeID in (4,8,9,10)  -- Monthly
					or (RecurringTypeID in (5,9,10) and convert(int,left(x.CurrentCY,2)) in (3,6,9,12))   -- Quarterly
					or (RecurringTypeID in (6,8,10) and convert(int,left(x.CurrentCY,2)) in (12))   -- Annual
				)
