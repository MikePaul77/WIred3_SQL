/****** Object:  Procedure [dbo].[VFPJobs_BuildProcessAdd]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE dbo.VFPJobs_BuildProcessAdd @AddJobType varchar(20)
AS
BEGIN
	SET NOCOUNT ON;
	...
	select @MaxJobID = max(JobID) from JobControl..Jobs
	declare @MaxQueryID int
	select @MaxQueryID = max(ID) from QueryEditor..Queries
	declare @MaxLayoutID int
	select @MaxLayoutID = max(ID) from JobControl..FileLayouts
	declare @MaxTemplateID int
	select @MaxTemplateID = max(ID) from JobControl..TransformationTemplates
...
select distinct JobID
	into #JobsToDo
	from (
			select JobID, Category
				from JobControl..VFPJobs_ListOfJobs2Import
				where JobType  = @AddJobType
		) x
				...
				begin
					...
					DECLARE curB CURSOR FOR
					 ...
					 BEGIN
							...
							if charindex('\',@StepFileNameAlgorithm) > 0
								set @StepFileNameAlgorithm =  right(@StepFileNameAlgorithm,charindex('\',reverse(@StepFileNameAlgorithm))-1)
...
---###############################################
	...
	DECLARE curB CURSOR FOR
		...
		select q.VFP_QueryID, max(coalesce(qtf.QueryTemplateID,fc.QueryTemplateID)), q.JobID, q.TemplateID, q.StepID
			from (
					select VFP_QueryID, JobID, null TemplateID, StepID 
					...
					if @NewCondType = 'Field'
						begin
							if isnumeric(@NewCondValue) = 1
								set @cmd = 'update QueryEditor..Queries set ' + @NewCondField + ' = ' + @NewCondValue + ' where TemplateID = ' + ltrim(str(@TemplateID)) + ' and VFPIn = 1 and ID > ' + ltrim(str(@MaxQueryID))
							else
								set @cmd = 'update QueryEditor..Queries set ' + @NewCondField + ' = ''' + @NewCondValue + ''' where TemplateID = ' + ltrim(str(@TemplateID)) + ' and VFPIn = 1 and ID > ' + ltrim(str(@MaxQueryID))
		 ...
		 insert QueryEditor..QueryLocations (QueryID, townID, VFPIn)
			select njs.QueryCriteriaID, t.Id, 1
				...
				where SELECTEXP2 = 'select2mtdtwn()'
					and njs.QueryCriteriaID > @MaxQueryID
...
--																						and pc.TransformTemplateID = ttc.TransformTemplateID
		 ...
		 --Update Sort Order with default of PropID or first Field
			update z set SortOrder = 1, z.SortDir = 'asc'
				...
				where z.FileLayoutID > @MaxLayoutID
			...
			-- Manual add Xform Columns
				insert JobControl..TransformationTemplateColumns (TransformTemplateID, OutName, SourceCode, VFPIn)
					select id, 'IsCondo', 'case when x.SalesPropUsage = 102 then 1 else 0 end', 1
						  from JobControl..TransformationTemplates
						  where ShortTransformationName = 'XPMTD'
							and ID > @MaxTemplateID
...
-- Specal Steps Control file Convert to Report
	...
	update jss set StepTypeID = 1101, StepFileNameAlgorithm = JobControl.dbo.VFPImport_FileExtSwap(jsr.StepFileNameAlgorithm, cf.FileExt)
		...
		where jss.StepTypeID in (24,1101)
			and jss.JobID > @MaxJobID

exec JobControl..VFPJobs_AdjustMergeJobs_Add @MaxJobID
				...
				BEGIN
					...
					update lf set SortOrder = coalesce(fn.Seq,0)
						...
						where lf.FileLayoutID = @SQLFileLayoutID
							and lf.FileLayoutID > @MaxLayoutID
...
-- End of Fix sort orders of Layouts that were not really used in VFP 
	...
	exec JobControl..VFPJobs_SyncQueryTemplateIDs_Add @MaxJobID
...
print 'R:\Wired3\Devel\Runable\QueryEditorSQLUpdate\QueryEditorSQLUpdate.exe XX'
select 'R:\Wired3\Devel\Runable\QueryEditorSQLUpdate\QueryEditorSQLUpdate.exe ' + Ltrim(str(ID)) 
	from QueryEditor..Queries where ID > @MaxQueryID
