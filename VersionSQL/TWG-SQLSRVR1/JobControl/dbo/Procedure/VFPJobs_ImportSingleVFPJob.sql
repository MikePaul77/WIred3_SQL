/****** Object:  Procedure [dbo].[VFPJobs_ImportSingleVFPJob]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE dbo.VFPJobs_ImportSingleVFPJob @InitVFPJobID int
AS
BEGIN
	SET NOCOUNT ON;
	...
	declare @JobExists bit = 1
	declare @MaxQueryID int = 0

	select @JobExists = case when count(*) > 0 then 1 else 0 end
	from JobControl..Jobs
	where VFPJobID = @InitVFPJobID

if @JobExists = 0
begin

	select @MaxQueryID = max(ID) from QueryEditor..Queries where VFPIn = 1
...
---###############################################
					...
					insert JobControl..VFPJobs_Params (JobID, ParamName, ParamVal)
						...
						union select @NewJobID, 'CountyID' ParmName, ltrim(str(c.Id)) ParmVal
								from DataLoad..Jobs_JOBs j
									join Lookups..States s on j.CURSTATE = s.Code
									join Lookups..Counties c on c.State_DONOTUSE = s.Code and c.Code = j.CCODE
								where j.CCODE > ''
									and j.jobID = @JobID
						union select @NewJobID, 'TownID' ParmName, ltrim(str(t.Id)) ParmVal
								from DataLoad..Jobs_JOBs j
									join Lookups..States s on j.CURSTATE = s.Code
									join Lookups..Towns t on t.State_DONOTUSE = s.Code and t.Code = j.TOWN
								where j.TOWN > ''
									and j.jobID = @JobID
...
---###############################################
	...
	DECLARE curB CURSOR FOR
		...
		select q.VFP_QueryID, max(coalesce(qtf.QueryTemplateID,fc.QueryTemplateID)), q.JobID, q.TemplateID, q.StepID
			from (
					select VFP_QueryID, js.JobID, null TemplateID, StepID
						from JobControl..JobSteps js
							join JobControl..Jobs j on j.JobID = js.JobID
						where js.VFPIn = 1 and VFP_QueryID > ''
							and j.VFPJobID = @InitVFPJobID
					) q
     ...
     BEGIN
			...
			select @NewQueryID = max(ID) from QueryEditor..Queries where VFPIn = 1 and ID > @MaxQueryID
	   ...
	   select distinct np.JobID, np.ParamName, np.ParamVal, 1 
			from JobControl..VFPJobs_Params np
				left outer join JobControl..JobParameters jp on np.JobID = jp.JobID
																...
																and jp.VFPIn = 1
				join JobControl..Jobs j on j.JobID = jp.JobID and j.VFPJobID = @InitVFPJobID
	...
	------------------------------------------------------------------------------------------------
	 ...
	 declare @VFPLayoutID varchar(50)
		...
		select jsX.VFP_LayoutID, Max(Q.TemplateID)
			...
			where jsQ.VFPIn = 1 and Q.ID > @MaxQueryID
...
--																						and pc.TransformTemplateID = ttc.TransformTemplateID
					where ttc.VFPIn = 1
						and ttc.TransformTemplateID = @NewXFromID
							...
							select * 
								...
								from (
									select FileLayoutID, RecID, case when SrcName = 'propid' then 1
										...
										where flf.VFPIn = 1
											and fl.TransformationTemplateId = @NewXFromID
				...
				insert JobControl..FileLayoutFields (FileLayoutID, VFPIn, OutOrder, SortDir, SrcName, OutName, OutType, OutWidth, Disabled, CaseChange, DoNotTrim, Padding, PaddingLength)
					select lf.FileLayoutID, 1, max(OutOrder)+1, '0', 'XF_IsCondo', 'IsCondo','bit',0,0,'None',0,'None',0
						...
						where l.ShortFileLayoutName = 'OUTMTD'
							and l.TransformationTemplateId = @NewXFromID
...
-- Add Deliver Steps
	...
	insert JobControl..JobSteps (JobID, StepName, StepTypeID, RunModeID, DeliveryTypeID, VFPIn, UpdatedStamp, CreatedStamp, SendDeliveryConfirmationEmail, StepOrder)
	 ...
	 -- Clean Up Duplicates in QueryEditor..QueryLocations 
		 ...
		 begin
			delete QR 
				from QueryEditor..QueryRanges QR
					join ( select QueryID, RangeSource, MinValue, MaxValue, Max(ID) MaxID
								from QueryEditor..QueryRanges
									where VFPIn = 1
										and QueryID > @MaxQueryID
							group by QueryID, RangeSource, MinValue, MaxValue
							having count(*) > 1 ) x on x.MaxID = QR.ID
...
--	update js set StepFileNameAlgorithm = replace(replace(js.StepFileNameAlgorithm,'<TBLN>',substring(replace(x.OUTTABLE,'\redp\output\Products\MTDE\',''), 1, len(replace(x.OUTTABLE,'\redp\output\Products\MTDE\','')) - 9)), '<IM2>','<TMY>')
	update js set StepFileNameAlgorithm = JobControl.dbo.VFPImport_TBLNCleanUpOT(js.StepFileNameAlgorithm,x.OUTTABLE)
		from JobControl..JobSteps js
			join JobControl..Jobs j on j.JobID = js.JobID
...
-- End of Fix sort orders of Layouts that were not really used in VFP 
	...
	Begin
		Print 'No Job Created, Looks like it allready exists.'

		select 'Job Exists', JobID, VFPJobID, JobName
			from JobControl..Jobs
			where VFPJobID = @InitVFPJobID
	end
